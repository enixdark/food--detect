...
def main(_):
    ...
    s3_download = False
    if "s3" in FLAGS.image_dir:
        dir = 'training_data'
        s3_link = FLAGS.image_dir
        if (os.name == 'nt'):
            subprocess.call(['C:\\Program Files\\Amazon\\AWSCLI\\aws.exe', 's3', 'cp', s3_link
                            , dir, '--recursive'], shell=True)
        else:
            subprocess.call(['.local/bin/aws', 's3', 'cp', s3_link
                                , dir, '--recursive'], shell=True)
        FLAGS.image_dir = os.path.join(os.getcwd(), dir)
        s3_download = True
    ...
    if s3_download:
        shutil.rmtree(FLAGS.image_dir, ignore_errors=True)
